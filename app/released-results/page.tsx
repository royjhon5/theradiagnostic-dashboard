...
import useGetReleased from "./hooks/useGetReleased";

export default function ClientResultEntryContainer() {
  const { releasedData } = useGetReleased();
  const result = globalClientForReview.parse(releasedData);
  return (
    <MainProvider>
      <div className="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
        <div className="px-4 lg:px-6">
          <div className="flex flex-col gap-1 text-blue-500 font-bold mb-2">
            <h2 className="text-2xl">Released Results</h2>
            <DynamicBreadcrumb />
          </div>
